...
namespace Footsteps {
	...
	public class CharacterFootsteps : MonoBehaviour {
		...
		void Start() {
            ...
            InitialiseAudioMixerGroup(GetComponent<AudioSource>());
		}
		...
		void Update() {
			...
			if (triggeredBy == TriggeredBy.TRAVELED_DISTANCE) {
                float speed = (p.desiredDirection).magnitude; /*(characterController ? characterController.velocity : characterRigidbody.velocity).magnitude;*/

				if(isGrounded) {
					// Advance the step cycle only if the character is grounded.
					AdvanceStepCycle(speed * Time.deltaTime);
                    //Debug.Log(p.desiredDirection.magnitude);
				}
			}
		}
